...
#kubectl wait -n "$NAMESPACE" --for=condition=complete --timeout=0 jobs/load-test
...
kubectl delete -n "$NAMESPACE" jobs load-test --force --grace-period=0
kubectl delete -n "$NAMESPACE" configmap load-test --force --grace-period=0
kubectl delete clusterrolebinding load-test --force --grace-period=0
